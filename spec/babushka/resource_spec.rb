...
describe Resource, '#content_subdir' do
  ...
  [
    ...
    [[],       ['test.app'],        nil],
    [[],       ['dir'],             'dir']
  ].each do |(files, dirs, result)|
    listing = files + dirs.map {|dir| dir + '/' }
    it "should return #{result.inspect} for #{listing}" do
      Dir.should_receive(:glob).with('*').and_return(listing)
      @resource.content_subdir.should == result
    end
  end
  ...
  [
    ...
    it "should return #{result.inspect} for #{listing}" do
      Dir.should_receive(:glob).with('*').and_return(listing)
      Dir.should_receive(:glob).with('*/').and_return(dirs)
      @resource.content_subdir.should == result
    end
  end
  ...
  #     @resource.content_subdir.should == 'contents'
  #   end
  # end
  # context "when there is more than one dir" do
  #   context "and none are named after the archive" do
  #     before {
  #       Dir.should_receive(:glob).with('*').and_return(['contents', 'another'])
  #       Dir.should_receive(:glob).with('*/').and_return(['contents', 'another'])
  #     }
  ...
  #     before {
  #       Dir.should_receive(:glob).with('*').and_return(['contents', 'another'])
  #       Dir.should_receive(:glob).with('*/').and_return(['contents', 'test'])
  #     }
  ...
  #   context "and none are named after the archive" do
  #     before {
  #       Dir.should_receive(:glob).with('*/').and_return(['contents', 'LaunchBar.app', 'RSpec.tmbundle'])
  #     }
  ...
  #   context "and one is named after the archive" do
  #     before {
  #       Dir.should_receive(:glob).with('*/').and_return(['contents', 'test.app'])
  #     }
  #     it "should not choose the non-descendable dir" do
  #       @resource.content_subdir.should be_nil
  #     end
  ...
  #     before {
  #       Dir.should_receive(:glob).with('*/').and_return(['contents', 'test.app', 'test'])
  #     }
